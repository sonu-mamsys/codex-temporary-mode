...
export function installVSCode(directory, helperSource) {
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    return 'Already patched; restore before changing the patch version.';
  }
  ...
  try {
    ...
    const manifest = { version: 3, files: files.map(e => ({ path: path.relative(root, e.file), original: digest(e.original), patched: digest(e.patched) })), helper: { path: path.relative(root, helper), hash: digest(helperSource) } };
    createExclusive(manifestPath, JSON.stringify(manifest, null, 2));
    return `Patched VS Code extension ${pkg.version}. Reload VS Code.`;
  } catch (error) {
...
function verifyManifest(root, manifest) {
  if (manifest.version !== 3 || !Array.isArray(manifest.files) || ![2, 3, 4].includes(manifest.files.length)) throw new Error('Invalid patch manifest. No files changed.');
