...
export const HELP = `GhostThread — Temporary Mode for Codex

Usage: ghostthread [options] [first prompt]
       ghostthread --verify [test prompt]
       ghostthread vscode install|restore [--vscode-path <directory>]
...
export async function main(args = process.argv.slice(2)) {
  if (args[0] === 'vscode') {
    ...
    if (!['install', 'restore'].includes(action)) throw new Error('Use ghostthread vscode install or ghostthread vscode restore.');
  ...
  const open = async () => {
    ...
    server = new CodexAppServer({ launch });
    const owned = server;
